...
flowchart TD
    ...
    L2 -->|"Pide descuento"| M1D{"¿Puede mejorar precio\ncon proveedor actual?"}
    M1D -->|"Sí"| M_OK["Extiende descuento al cliente."]
    M1D -->|"No"| M2D{"¿Hay proveedor\nalternativo con mejor precio?"}
    M2D -->|"Sí"| M_OK
    M2D -->|"No"| M3D["Solicita autorización a Javier Pablos\n(sacrificar margen de utilidad)."]
    M3D --> M4D{"¿Javier\nautoriza descuento?"}
    M4D -->|"Sí"| M_OK
    M4D -->|"No"| M_NO["Comunica al cliente: no hay\nmás descuento disponible."]
    M_OK --> M_QUOT["Genera nueva cotización\ncon descuento en Odoo."]
    M_QUOT --> K
    M_NO --> K
    ...
    style M_QUOT fill:#6a0dad,color:#fff
    style M1D fill:#f0c040,color:#000
    style M2D fill:#f0c040,color:#000
    style M4D fill:#f0c040,color:#000
